...
export class ArticleComponent {
  ...
  openUpdateModal(article: Article): void {
    ...
    this.selectedArticle = { ...article }; // Copie complète pour éviter les références directes
  ...
  deleteArticle(): void {
    this.articleService.deleteArticle(this.currentArticleId).subscribe(
      () => {
        // Supprimer l'article localement après la confirmation de la suppression dans la base de données
      ...
      (error) => {
        ...
        Swal.fire({
          title: 'Erreur',
          text: 'Une erreur est survenue lors de la suppression de l\'article. Veuillez réessayer.',
          icon: 'error',
          confirmButtonText: 'OK',
        });
      }
  ...
  updateArticle(): void {
    if (!this.selectedArticle.id || !this.selectedArticle.titre || !this.selectedArticle.contenu || !this.selectedArticle.plante || this.selectedArticle.plante.id <= 0) {
      Swal.fire({
        title: 'Erreur',
        text: 'Tous les champs sont obligatoires pour modifier l\'article.',
        icon: 'error',
        confirmButtonText: 'OK',
      });
      return;
    ...
    formData.append('planteId', this.selectedArticle.plante.id.toString());  // Pour envoyer l'objet plante
    ...
    this.articleService.updateArticle(this.selectedArticle.id, formData).subscribe(
      (updatedArticle: Article) => {
        ...
        if (index !== -1) {
          this.articles[index] = { ...updatedArticle }; // Mise à jour locale
          this.filteredArticles = [...this.articles];
        }
      ...
      (error) => {
        ...
        Swal.fire({
          title: 'Erreur',
          text: 'Impossible de mettre à jour l\'article.',
          icon: 'error',
          confirmButtonText: 'OK',
        });
      }
